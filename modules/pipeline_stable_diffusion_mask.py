...
# Modified by JIANGJiaXiu on 2024-04-01
...
class StableDiffusionPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def __call__(
        ...
        mask_list: List[torch.Tensor] = None,  # each mask should be 2D tensor
        ...
        **kwargs,
        ...
        if mask_list is not None:
            mask_list = [mask.unsqueeze(0).unsqueeze(0) for mask in mask_list]  # 2D -> 4D
            mask_list = [F.interpolate(mask, size=64) for mask in mask_list]  # resize to 64x64
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    ...
                    # perform masking
                    if mask_list is not None:
                        noise_pred_text = mask * noise_pred_text + (1.0 - mask) * noise_pred_uncond
                    else:
...
if __name__ == '__main__':
    ...
    pipe = StableDiffusionPipeline.from_single_file(sd_path, safety_checker=None)
    pipe1 = StableDiffusionPipeline.from_single_file(sd_path, safety_checker=None)
    pipe2 = StableDiffusionPipeline.from_single_file(sd_path, safety_checker=None)
    ...
    # TODO: construct your masks here!
    mask0 = torch.ones(64, 64)
    mask1 = torch.ones(64, 64)
    ...
    image = pipe(prompt, cfg_scale, weights,
